...
-- Show notification when script is loaded
StarterGui:SetCore("SendNotification", {
    Title = "Loaded!";
    Text = "LUCK INCREASED 1000000X";
    Duration = 5;
})
...
for _, p in ipairs(Players:GetPlayers()) do
    if p ~= localPlayer and p.Character and p.Team then
        if p.Team.Name == "Killer" then
            if Hook.Players["Killer"].On then
                ESP(p.Character, Hook.Players["Killer"].Color)
            end
        elseif p.Team.Name == "Survivors" or p.Team.Name == "Survivor" then
            if Hook.Players["Survivor"].On then
